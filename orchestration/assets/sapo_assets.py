...
def sapo_customers_batch_asset(context):
    ...
    Writes to ingestion_health via orchestration.ops.ingestion_health.
    """
    asset_key_str = "sapo/sapo_customers_batch_asset"
    started = datetime.now(timezone.utc)
    is_full_refresh = context.run.tags.get("full_refresh") == "true"
...
def sapo_accounts_batch_asset(context):
    ...
    Writes to ingestion_health via orchestration.ops.ingestion_health.
    """
    asset_key_str = "sapo/sapo_accounts_batch_asset"
    started = datetime.now(timezone.utc)
    is_full_refresh = context.run.tags.get("full_refresh") == "true"
    argv = ["--full-refresh"] if is_full_refresh else []
...
def sapo_products_batch_asset(context):
    ...
    Writes to ingestion_health via orchestration.ops.ingestion_health.
    """
    asset_key_str = "sapo/sapo_products_batch_asset"
    started = datetime.now(timezone.utc)
    is_full_refresh = context.run.tags.get("full_refresh") == "true"
    argv = ["--full-refresh"] if is_full_refresh else []
...
def sapo_history_log_asset(context):
    ...
    Writes to ingestion_health via orchestration.ops.ingestion_health.
    """
    asset_key_str = "sapo/sapo_history_log_asset"
    started = datetime.now(timezone.utc)
...
def sapo_webhook_consumer_asset(context):
    ...
    Writes to ingestion_health via orchestration.ops.ingestion_health.
    """
    asset_key_str = "sapo/sapo_webhook_consumer_asset"
    started = datetime.now(timezone.utc)
