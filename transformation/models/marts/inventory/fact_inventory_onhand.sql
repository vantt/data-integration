...
-- =================================================================================================
-- FACT: INVENTORY ON-HAND (Sparse effective-dated balance ledger)
-- =================================================================================================
...
--
-- Replaces the former dense daily carry-forward (fact_inventory_onhand_daily, ~1.9M rows, of which
-- 99.1% were redundant no-movement carry-forward duplicates). This sparse ledger materializes ONLY
-- days where on-hand actually changed (~17.6k rows) and exposes an effective date range, so any
-- point-in-time query works WITHOUT storing every calendar day:
...
--   - Negative on-hand pairs flagged via is_negative_onhand.
--   - Multiple movements on the same day: day-close = the movement with the latest issued_at.
--
-- Name note: fact_inventory_ONHAND (not _snapshot) avoids collision with the unrelated
--   fact_inventory_snapshot (Sapo product-batch nightly pull, models/marts/sales/).
-- =================================================================================================
