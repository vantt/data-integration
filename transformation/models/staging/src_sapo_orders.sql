...
--   4. Business dedup by order_id (latest event_timestamp + modified_on wins).
--   5. Discard payload → frees memory for downstream models.
--
-- OOM-safe:
--   - Tech dedup ROW_NUMBER sorts on lightweight keys (entity_id, event_timestamp).
...
--   - Incremental processes only 7-day window → small dataset.
-- =================================================================================================
...
-- Step 2: Business dedup by order_id — operates on flat data only, no payload
