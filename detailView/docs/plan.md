...
- No auth. LAN-only behind Caddy (`detailview.lan.fwg.vn`). Runs as a Docker Compose service like metabase/rill.

## Decisions (confirmed)
| # | Decision | Choice |
|---|---|---|
| 1 | Stack | FastAPI + Jinja2 + **HTMX now** (tab/search partial loads). No Node build. |
| 2 | App name | folder `detailView/`; docker service/container `detail_view`; Caddy `detailview.lan.fwg.vn`; port `3005:8000` |
