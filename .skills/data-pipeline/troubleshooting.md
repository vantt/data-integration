...
| **Realtime job stuck lặp lại liên tục** (Runtime: 10 min, Inactive: ~9 min, mỗi 3-13 min) | Zombie dbt subprocess từ run trước đang giữ DuckDB WAL lock — `finally: watchdog.cancel()` disarm watchdog khi alerter cancel run, `_terminate_subprocess_tree` không tìm được dbt process (không có run_id trong env) | Fix: `finally` block phải kill `invocation.process` nếu vẫn còn sống — xem L60. Sau fix, restart Docker: `docker compose restart data_platform` |
| **Job NOT_STARTED 90+ min dù không có job nào chạy** | Daemon coordinator bị slow/freeze do zombie processes tích lũy (xem trên) hoặc SQLite contention | Fix L60 trước (ngăn zombie). Check daemon log: `docker compose logs data_platform \| grep QueuedRunCoordinator`. Nếu daemon healthy → giảm QUEUE_STUCK_THRESHOLD (xem L61) |
