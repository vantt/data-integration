...
Job Selection Guide:
  ...
  transform_batch_nightly_job       → batch sync: orders, customers, accounts (04:00 AM)
  transform_batch_fullrefresh_job   → manual one-time full reload (tag "full_refresh=true" baked in)
...
QUAN TRỌNG — Full-refresh design (xem L32):
  - transform_batch_nightly_job KHÔNG có full_refresh tag → chạy incremental bình thường
  - transform_batch_fullrefresh_job có tag {"full_refresh": "true"} baked in → asset tự đọc tag, truyền argv
...
def {SOURCE}_{ENTITY}_batch_asset(context):
    ...
    Full-refresh: khi job được launch với tag {"full_refresh": "true"} (transform_batch_fullrefresh_job).
    """
    context.log.info("Starting {SOURCE} {ENTITY} Batch Sync...")

    # Đọc full_refresh từ run tag — set bởi transform_batch_fullrefresh_job definition
