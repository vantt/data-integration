...
16. **Nightly reconciliation = incremental, KHÔNG phải full refresh.** Dùng `sapo_full_refresh_job` (manual, tag baked in) cho one-time reload. Không bao giờ auto-tag `full_refresh=true` trên scheduled jobs. Batch source functions phải wire `full_refresh` param từ entry-point → source → resource — nếu không flag bị silently ignored. Xem L32.
