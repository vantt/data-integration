...
if [ -d "${PROJECT_ROOT}/app_data" ]; then
    ...
    for vol_name in data_lake dagster_home input_source; do  # crm_data removed: now backed up by crm/ops/backup_crm.py (consistent SQLite snapshot, not raw WAL copy)
