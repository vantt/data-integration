...
5. Separately, note (do not fix — out of this phase's scope) that `GET /tasks/modal/create`'s target template `fragments/modal_create_task.html` doesn't exist — flag this in the implementation report as a pre-existing bug for the user to decide on later, don't silently leave it undiscovered again. **Resolved in a same-plan follow-up session (2026-07-11)**: user asked to close it — fixed by retargeting S07 Tasks Board's "+ Tạo task" buttons to the existing M05 modal (`/modals/m05`) instead of authoring a new near-duplicate template; dead route + template reference removed. See `plan.md`'s "Follow-up session" log.
