...
| 03 | `phase-03-cost-taxonomy-promo-642-dedup.md` | `promo_goods_cost` (revenue=0 split, gift-no-invoice); cross-tier **642 count-once** rule | ✅ DONE — `int_order_promo_goods_cost` (1,339 lines/464M, strict line_revenue=0, gift-no-invoice flag) + `int_promo_642_monthly_total` (Σ=1.08B, count-once helper for ph-04) + fact_order_costs PROMO_GOODS rows + fact_order_economics promo_goods_cost col. Verified Dagster + 12 tests. Threshold Q3=strict 0 (data: rev∈{0,≥1000}). |
| 04 | `phase-04-overhead-allocation.md` | `int_order_overhead_allocation` (closure-based pools/base) + `fully_loaded_net_profit` | ✅ DONE — account-ledger ingest + classification gsheet (nightly) + `int_overhead_pool_monthly` + `int_order_overhead_allocation` (closure exact) + facts (`allocated_overhead`/`fully_loaded_net_profit` + OVERHEAD rows). Verified Dagster. P4-5 reconcile verified 2026-06-09. P4-3 provisional branch → deferred to [260609-1343-overhead-budgeted-rate-open-month](../../260609-1343-overhead-budgeted-rate-open-month/plan.md) |
| 05 | `phase-05-pl-marts-serving.md` | unified `fact_order_economics`/`fact_order_costs` columns + serving views + Metabase P&L | ✅ DONE — all dbt/serving/Dagster green; Metabase "Fully Loaded P&L" tab deployed 2026-06-09 |
| 06 | `phase-06-detailview-pl.md` | detailView per-line COGS/margin + reconciliation panel + full P&L (coordinate w/ concurrent detailView work) | ✅ DONE — CogsItem per-SKU pattern, all domain/adapter/template changes, verified |
