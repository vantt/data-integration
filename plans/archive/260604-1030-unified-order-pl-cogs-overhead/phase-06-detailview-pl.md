...
**Status:** DONE — implemented as CogsItem (per-SKU) pattern instead of CogsReconPanel (order-level); functionally equivalent. order_cogs_items.sql queries int_* directly (minor arch deviation, acceptable KISS tradeoff).
...
- [x] Verify concurrent detailView customer stream merged (git log check)
- [x] Verify phase 05 Dagster-green + serving columns present (step 1)
- [x] Confirm `variant_id` in `order_line_items.sql`; add if missing (step 2)
- [x] Create `order_line_cogs.sql` (step 3 — implemented as `order_cogs_items.sql` querying int_order_cogs_reconciled + int_order_promo_goods_cost)
- [x] Create `order_cogs_recon.sql` (step 4 — per-SKU CogsItem pattern used instead of order-level CogsReconPanel)
- [x] Update `order.py` — 6 new `OrderFinancial` fields, `LineItem.cogs_amount`, `CogsItem` dataclass (replaces CogsReconPanel), `OrderDetail.cogs_items`, `quality_flags` update, `margin_is_verified` update (step 5)
- [x] Update `order_header.sql` — 6 new columns (step 6)
- [x] Update `order_mappers.py` — `map_financial`, `map_cogs_item` (step 7)
- [x] Update `order_repository.py` — fetch cogs_items via `order_cogs_items` query (step 8)
- [x] Update HTML template — waterfall panel (_financial.html), cost ledger badges + PROMO_GOODS/OVERHEAD styling (_cost_ledger.html) (step 9)
- [x] detailView manual integration test passes (step 10)
- [x] Full Dagster nightly run SUCCESS (step 11)
