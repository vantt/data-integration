...
- [x] Phase 02 GREEN (gate check)
- [x] Concurrent stream coordination confirmed
- [x] Verify `std_order_items.sku` column presence (or identify variant_id→sku join)
- [x] Create `int_order_promo_goods_cost.sql`
- [x] Create `int_promo_642_monthly_total.sql`
- [x] Extend `fact_order_costs.sql` — add promo_goods CTE + UNION ALL
- [x] Extend `fact_order_economics.sql` — add order_promo CTE + promo_goods_cost column
- [x] `dbt compile` — no errors
- [x] `dbt build` — full chain green
- [x] Validate promo split totals (SUM ~1.08B in has_misa_642 subset)
- [x] Validate gift-no-invoice count / amount
- [x] Validate no double-count (cogs + promo = pre-BUG-1 total)
- [x] `int_promo_642_monthly_total` spot-check (period totals reasonable)
- [x] Dagster manual run → SUCCESS
- [x] Hand-off note to phase 04: document whether expense ledger includes sales-ledger-642
