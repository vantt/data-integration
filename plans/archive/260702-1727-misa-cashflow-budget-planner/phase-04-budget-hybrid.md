...
Google Sheet (finance nhập đầu mỗi tháng, ~15-20 phút)
  ...
  │           reserve (target+deadline) → gap_còn_lại / months_until_target
  │           reserve (target only)     → không tính gợi ý; hiện "X/target"
  │           reserve (open-ended)      → không tính gợi ý; finance nhập tay
...
| `item_target` | double | Target tích lũy (nullable) — xem validation bên dưới |
| `target_month` | date | Tháng deadline (nullable) — xem validation bên dưới |

**Validation target/deadline (script reject nếu vi phạm):**
- `target_month IS NOT NULL AND item_target IS NULL` → **REJECT**: deadline không có target là vô nghĩa
- `item_target IS NOT NULL AND target_month IS NULL` → OK: tích lũy đến khi đủ, không áp lực thời gian
- Cả hai NULL → OK: open-ended, finance nhập tay từng tháng
- Cả hai có giá trị → OK: sinking fund đầy đủ, script tính `gap/months_until_target`
...
WITH budget AS (
    -- Aggregate across item_types: fact_cashflow_budget có nhiều rows per (cashflow_line, period_month, direction)
    -- (recurring, reserve, one_off trong cùng 1 cashflow_line)
...
- `priority`: số nguyên dương, nhỏ = ưu tiên cao. **Constraint cứng: `remainder` phải là số lớn nhất (cuối cùng)** — script reject nếu vi phạm.
...
**Validation ALLOCATION_POLICY (script enforce trước khi generate CSV):**
1. `remainder` phải là `priority` cao nhất trong current rows (effective_to IS NULL) — reject nếu có bucket nào sau nó
2. Không có gap hoặc overlap trong `effective_from`/`effective_to` cho cùng 1 bucket
3. `rule_type IN ('fill_to_target', 'from_plan', 'fixed', 'pct_remaining', 'remainder')`
4. `value` bắt buộc khi `rule_type IN ('fill_to_target', 'fixed', 'pct_remaining')`, null khi `from_plan/remainder`

**Google Apps Script validation (in-Sheet):**
Xem `scripts/budget/validate-budget-sheet.gs` — runs on edit + on-demand menu item:
- BUDGET_ITEMS: dropdown enforcement (Dòng tiền), item_type values, target/deadline rules
- ALLOCATION_POLICY: rule_type values, remainder last, no gap/overlap
