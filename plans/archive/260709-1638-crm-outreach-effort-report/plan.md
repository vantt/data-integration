...
9. ~~*(2026-07-11, phát hiện khi đóng Q#7)* Verify Q#7 lộ ra 1 bug hạ tầng có thật...~~ **ĐÓNG 2026-07-11** — thêm column-level schema-drift check vào `refresh_rolling.py`: đọc schema file rolling mới nhất qua in-memory DuckDB connection (không đụng serving DB đang lock), so với schema đã lưu lần trước; nếu 1 cột ĐÃ TỒN TẠI đổi TYPE (không phải cột mới thêm) → in `[!] SCHEMA_DRIFT` (Dagster asset đã sẵn regex bắt marker này, chỉ generalize lại message text vì giờ có 2 loại drift). Verify thật trên 94 mart production (baseline sạch, chạy lại lần 2 im lặng đúng kỳ vọng) + review độc lập (8/8 test PASS, không có breaking caller nào của `load_known_tables()`/`save_known_tables()` dù đổi return signature).
