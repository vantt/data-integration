...
- Compose service `detail_view`: build context `.`, port `3005:8000`, `env_file: .env.docker`, network `caddy_net`, mount `./app_data/data_lake:/app/var/data_lake:ro`, labels `caddy: detail.local` + `reverse_proxy {{upstreams 8000}}`.
...
- AC-6: Every source file < 200 LOC; runs in container at `detail.local`.
