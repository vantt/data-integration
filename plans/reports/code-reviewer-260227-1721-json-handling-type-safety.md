...
|---|-----------|--------|----------|
| 1 | Malformed JSON in payload | ⚠️ Partial | Medium |
| 2 | Null/missing JSON fields | ⚠️ Partial | Low-Medium |
| 3 | Empty array unnesting / data loss | ⚠️ Partial + build-breaking duplicates | **High** |
| 4 | Surrogate key with NULL columns | ✅ Handled (2 edge cases remain) | Medium |
| 5 | Unmapped CASE status fall-through | ✅ Handled | Low |
| 6 | Type mismatch on JSON extractions | ✅ Handled | Low |
...
1. **[Critical] Fix duplicate column definitions** in `stg_sapo_fulfillments.sql`, `stg_sapo_payments.sql`, `stg_sapo_order_items.sql` — remove the duplicated lines.
2. **[High] Handle empty array case** in all three unnest models: add `AND json_extract_string(payload, '$.fulfillments') != '[]'` and `!= 'null'` to WHERE filters.
3. **[Medium] Add coalesce to `fact_payments.sql`** surrogate key calls for `payment_id` and `payment_method_id` to prevent hash collisions on NULL values.
4. **[Medium] Add coalesce to `fact_orders.sql` line 42** for `location_id` surrogate key.
5. **[Low] Add dbt `not_null` tests** on `order_id` in `stg_sapo_orders` and `amount` in `stg_sapo_payments` to surface silent TRY_CAST failures.
6. **[Low] Consider wrapping payload in `TRY_CAST(payload AS JSON)`** in `stg_sapo_orders` and `stg_sapo_customers` to isolate malformed JSON rows before extraction.
