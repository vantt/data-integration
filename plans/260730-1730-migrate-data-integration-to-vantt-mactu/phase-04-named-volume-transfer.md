...
3. Transfer tar files qua ssh:
   ```bash
   scp .migrate-staging/*.tar vantt-mactu:~/data-integration/.migrate-staging/
   ```
4. Tạo named volume mới trên vantt-mactu và load (KHÔNG dùng `docker compose up` trước — tạo volume trần trước để load data vào, tránh compose tự tạo volume rỗng rồi container start sớm):
   ```bash
   ssh vantt-mactu bash -s <<'EOF'
   for VOL in dagster_home monitoring_db crm_data crm_backups; do
     docker volume create "data-integration_${VOL}"
     docker run --rm \
       -v "data-integration_${VOL}:/dest" \
       -v ~/data-integration/.migrate-staging:/src:ro \
...
5. Verify size khớp (đã đo baseline: dagster_home 9.8G, monitoring_db 54.9M, crm_data 57.5M, crm_backups 303M):
   ```bash
   ssh vantt-mactu 'MSYS_NO_PATHCONV=1 docker run --rm -v data-integration_dagster_home:/v1 -v data-integration_monitoring_db:/v2 -v data-integration_crm_data:/v3 -v data-integration_crm_backups:/v4 alpine du -sh /v1 /v2 /v3 /v4'
   ```
6. Verify integrity CRM SQLite (có `PRAGMA integrity_check` thật, khác DuckDB/H2):
   ```bash
   ssh vantt-mactu 'docker run --rm -v data-integration_crm_data:/d alpine sh -c "apk add -q sqlite && sqlite3 /d/crm.db \"PRAGMA integrity_check\" && sqlite3 /d/cache.db \"PRAGMA integrity_check\""'
   ```
...
## Rollback
Windows: chỉ dừng/start lại, không mất gì. vantt-mactu: `docker volume rm data-integration_{dagster_home,monitoring_db,crm_data,crm_backups}` rồi làm lại nếu integrity check fail.
