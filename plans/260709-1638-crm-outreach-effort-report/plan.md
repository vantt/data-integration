...
7. ~~*(2026-07-11, phát hiện lúc sync-back)* `zalo_connected_count` chưa có cột nào trong `mart_staff_performance_weekly` dù UI+export đã sẵn sàng...~~ **ĐÓNG 2026-07-11** — thêm `COUNT(*) FILTER (WHERE al.zalo_connected)` vào CTE `calls_funnel`, verify dbt run/test PASS + cột sống trong serving view (sum=0, đúng vì tính năng vừa ship, chưa ai dùng checkbox thật).
8. ~~*(2026-07-11, phát hiện lúc sync-back)* `crm_party_tag.source_activity_id` write-path đã sẵn...~~ **ĐÓNG 2026-07-11** — hướng (a) giờ rẻ hơn ước tính gốc vì Phase 3 (disposition strip) đã có sẵn `S.draftId` client-side; chỉ cần thêm `source_activity_id: S.draftId || ''` vào `s14TagMultiSave`'s POST values (1 dòng). Route/service layer không đổi (đã sẵn sàng từ phase 1). 2 test route mới (`test_source_activity_id_passthrough_when_provided`, `test_source_activity_id_blank_stays_none`).
9. *(2026-07-11, phát hiện khi đóng Q#7)* Verify Q#7 lộ ra 1 bug hạ tầng có thật, KHÔNG liên quan riêng `zalo_connected_count`: `scripts/provisioning/bootstrap_serving_views.py`'s `read_parquet(glob, ...)` thiếu `union_by_name=true` — khi rolling folder có ≥2 file khác schema cùng lúc (luôn xảy ra 1-2 chu kỳ sau MỌI lần thêm cột mart, vì `refresh_rolling.py` giữ tối đa `ROLLING_KEEP_VERSIONS=3` bản không phân biệt schema), view có thể âm thầm dùng schema CŨ, mất cột mới dù bootstrap "thành công". Đã fix (`union_by_name=true`), ảnh hưởng MỌI mart chứ không riêng plan này. Review độc lập verify bằng thực nghiệm: fix đúng cho cột thêm mới, nhưng đổi lỗi "loud" (Conversion Error, bootstrap log rõ ràng) thành "silent" (âm thầm coerce type) cho trường hợp hiếm hơn — đổi TYPE 1 cột giữa các bản rolling. Chưa gặp trường hợp đó trong repo (marts hiện chỉ THÊM cột, không đổi type), nhưng backlog: nên thêm column-level schema-drift check vào `refresh_rolling.py` (hiện chỉ so tên bảng, không so cột/type).
