...
- [ ] Create `src_sapo_product_variants.sql` (Step 1)
- [ ] Create `seed_sku_alias.csv` with pre-filled known aliases (Step 2)
- [ ] Create `dim_sku_alias.sql` (Step 3)
- [ ] Create `dim_product_variants.sql` (Step 4)
- [ ] Modify `dim_products.sql` to union catalog + order_items (Step 5)
- [ ] Add schema.yml tests (not_null, unique) for new models
- [ ] Wait for ingestion fix (agent af4df…) then run: `dbt build --select +dim_products +dim_product_variants +dim_sku_alias`
- [ ] Verify MISA hit rate: query `mart_sku_economics_monthly` — expect COGS coverage ~85%
- [ ] Verify no downstream model breakage (fact_sales, fact_order_economics join on product_key)
- [ ] Request business owner review of seed_sku_alias.csv `pending` entries
