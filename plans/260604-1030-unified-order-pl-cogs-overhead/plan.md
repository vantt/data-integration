...
| 04 | `phase-04-overhead-allocation.md` | `int_order_overhead_allocation` (closure-based pools/base) + `fully_loaded_net_profit` | ✅ CORE DONE — account-ledger ingest + classification gsheet (nightly) + `int_overhead_pool_monthly` + `int_order_overhead_allocation` (closure exact) + facts (`allocated_overhead`/`fully_loaded_net_profit` + OVERHEAD rows). Verified Dagster. Còn: P4-3 provisional (live-month, data hiện actual) + P4-5 reconcile 64214 vs sales-642 |
