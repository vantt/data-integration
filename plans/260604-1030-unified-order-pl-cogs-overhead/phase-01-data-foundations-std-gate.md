...
- [ ] Pause Dagster schedules before dev build
- [ ] Create `std_misa_sales_lines.sql`
- [ ] Update `schema.yml` with model + column docs
- [ ] Repoint `int_misa_sales_lines.sql` (one ref change)
- [ ] `dbt compile` check — no errors
- [ ] `dbt build` — std + int + fact chain green
- [ ] Dagster manual run → SUCCESS
- [ ] Smoke-check: cost_account_group distribution matches design §1a (~95%/632, ~4.8%/642)
- [ ] Document overhead schema decisions in design doc
