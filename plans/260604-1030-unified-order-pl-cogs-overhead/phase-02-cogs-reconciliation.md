...
- [ ] Phase 01 GREEN (gate check)
- [ ] Concurrent stream coordination confirmed (no open edits on fact_order_economics / fact_order_costs)
- [ ] Add `cost_account_group` to `int_misa_sales_lines` SELECT
- [ ] Create `intermediate/cogs/` directory
- [ ] Create `int_order_cogs_reconciled.sql`
- [ ] BUG-1: fix `fact_order_economics.sql` misa_order CTE (add cost_account_group='632' filter)
- [ ] BUG-1: fix `fact_order_costs.sql` cogs CTE (add cost_account_group='632' filter)
- [ ] `dbt compile` — no errors
- [ ] `dbt build` — all green
- [ ] BUG-1 delta verified (~+1.08B COGS drop from economics total)
- [ ] `int_order_cogs_reconciled` cogs_source distribution validated
- [ ] Dagster manual run → SUCCESS
- [ ] Resume Dagster schedules
