...
- **Insight App:** FastAPI + Jinja2 + HTMX — `detailView`, read-only order/customer detail pages (hexagonal; reads `olap.duckdb` read-only; Docker service `detail_view` @ `detail.local`)
