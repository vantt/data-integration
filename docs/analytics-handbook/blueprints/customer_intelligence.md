...
# 📘 Blueprint: Monthly · Customer Intelligence [Cross]
...
Monthly strategic review — customer value, segments, behavior, acquisition. Audience CEO/CMO.

---

### 🖥️ Dashboard: Monthly · Customer Intelligence [Cross]

**Description**: Monthly deep-dive — customer health scorecard, value concentration, segment dynamics, purchase behavior, channel effectiveness, acquisition quality, and geographic distribution across 3 focused tabs.
...
GROUP BY 1
ORDER BY
    CASE COALESCE(discount_sensitivity, 'Unknown / Insufficient data')
...
ORDER BY
    CASE value_group
        ...
        ELSE 4
    END,
    CASE COALESCE(discount_sensitivity, 'Unknown')
        WHEN 'PROMO_DEPENDENT' THEN 1
        WHEN 'PROMO_MIXED'     THEN 2
        WHEN 'FULL_PRICE'      THEN 3
        ELSE 4
...
#### 📝 Text: Geographic view — where are customers and value concentrated?

# Geographic view — where are customers and value concentrated?

```json metabase-pos
{ "row": 43, "col": 0, "size_x": 18, "size_y": 1 }
```
...
```json metabase-pos
{ "row": 44, "col": 0, "size_x": 9, "size_y": 8 }
```
...
```json metabase-pos
{ "row": 44, "col": 9, "size_x": 9, "size_y": 8 }
```
