...
**Description**: Công cụ đối soát đơn hàng — Reconciliation Checklist + Data Freshness + KPI + phân bổ (donut/bar) + cảnh báo bất thường + chi tiết đơn. Chọn ngày qua filter "Ngày".
...
```sql
SELECT '📅 Ngày: ' || strftime((SELECT MIN(ordered_at)::DATE FROM fact_orders WHERE {{ordered_at}}), '%d/%m/%Y') AS "Chu kỳ báo cáo"
```
...
3. **Quét Flagged Orders** — bất kỳ dòng nào cũng cần xác minh Sapo.
4. **Lệch > 1 đơn?** Mở Order Detail List, search order code trên Sapo → báo Data Team nếu là ingestion gap.
...
WHERE o.order_id IN (SELECT order_id FROM fact_orders WHERE {{ordered_at}})
GROUP BY 1
ORDER BY 3 DESC
```
