...
combo, or pivot component (see Deviations below).
...
|---|---|---|
| `period_month` field filter (interactive) | Fixed windows: KPIs = latest closed month vs prior month; trends = 12-month rolling | Evidence pages are a build-time snapshot, no live filter widget |
| `waterfall` display | `BarChart swapXY` + `DataTable` | No waterfall component in Evidence |
