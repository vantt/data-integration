...
SELECT
    ...
    || '  ·  Shopee payout cuối: ' || COALESCE(strftime(timezone('Asia/Ho_Chi_Minh', (SELECT MAX(payout_released_at) FROM int_shopee_order_fees WHERE payout_released_at IS NOT NULL)), '%d/%m'), 'chưa có')
    AS "Độ tươi dữ liệu"
FROM fact_orders o
