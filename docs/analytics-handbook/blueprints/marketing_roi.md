...
[[WHERE date_key >= CAST(strftime({{date_range}}, '%Y%m%d') AS INTEGER)]]
```
...
WHERE o.status = 'COMPLETED'
  ...
  [[AND o.date_key >= CAST(strftime({{date_range}}, '%Y%m%d') AS INTEGER)]]
```
...
WITH monthly_spend AS (
    ...
    [[WHERE m.date_key >= CAST(strftime({{date_range}}, '%Y%m%d') AS INTEGER)]]
