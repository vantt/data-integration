SELECT
    s.order_id,
    s.order_line_id,
    s.ordered_at AS sale_timestamp,
    date_trunc('day', s.ordered_at) AS sale_date,
    EXTRACT(HOUR FROM s.ordered_at) AS sale_hour,
    strftime(s.ordered_at, '%a') AS day_of_week,
    p.product_name,
    p.variant_name,
    p.sku,
    p.barcode,
