...
- **Sheet chips gave zero visual feedback on pick (2nd round of live testing, same session)**: `s14StripPickFollowup` (answered sheet's `+7/+14/+30 ngày`) and `s14StripPickCallbackChip` (callback sheet's `+2h`/`chiều nay`/`mai sáng`) both wrote their state correctly but never toggled `chip-pill--on` on the clicked button — unlike `s14StripPickReason` (refused sheet), which already did. Clicking read as "the button doesn't work" even though the pick was recorded. Fixed by mirroring `s14StripPickReason`'s highlight-on-click pattern in both functions (each button's `onclick` now also passes `this`).
- **`.btn--secondary`'s border visually collided with `.chip-pill--on`**: the answered sheet's "🛒 Đã mua → nhập mã đơn" button used `.btn--secondary`, whose accent-colored border is visually identical to a *selected* chip's border — sitting next to the 3 real date chips, it read as an already-picked 4th option. Switched to `.s14-strip__link` (the plain-link style already used for "⋯ Chi tiết" elsewhere in this sheet), since it performs a distinct action (switch outcome to `purchased`), not a date choice.
