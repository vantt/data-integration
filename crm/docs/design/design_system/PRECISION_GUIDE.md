...
| `ui_kits/crm/` | Order &amp; customer insight detail view. |
| `ui_kits/content-reading/` | Long-form markdown / documentation reading shell — chapters rail, centred reading column, scroll-spy TOC. |
| `ref/` | Original source files imported from the Claude Design exploration project. |

## Long-form reading layer (v0.2)

The UI scale (`--fs-body` 15 px / lh 1.55) is tuned for chrome. Reading surfaces — docs, articles, rendered markdown — use a dedicated layer added to `colors_and_type.css`, distilled from an editorial composer (centred column · eyebrow → title → lede · 17 / 1.7 body · tonal callouts · dark code) and set in Precision's Fraunces + Geist.

- **Reading scale:** `--fs-title` 40, `--fs-lede` 20, `--fs-prose-h2` 28, `--fs-prose-h3` 20, `--fs-prose` 17, `--fs-prose-code` 14.5.
- **Leading:** `--lh-prose` 1.7 (body), `--lh-lede` 1.5, `--lh-prose-head` 1.2.
- **Measure & rhythm:** `--measure` 68ch, `--reading-col` 720px, `--flow` 1.35em (block gap), `--flow-loose` 2.1em (before a section head), `--flow-tight` 0.5em (heading↔body, list items).
- **`.prose` engine:** wrap rendered markdown in `.prose` and a full **H1–H4** ramp (Fraunces h1 34 / h2 28 / h3 20, Geist-Mono-caps h4), body (Geist), lists, **definition lists**, links, inline/block code, blockquote (Fraunces-italic pull) and an `.aside` quote variant, `hr` (dot rule), figures and tables all pick up the rhythm with no per-element markup.
- **Inline marks:** `kbd`, `mark`, `del`/`ins`, `sub`/`sup`, `abbr`, `small`.
- **Block furniture:** `.callout` in five tones (amber default · `--moss` · `--coral` · `--honey` · `--note` neutral) and `.codeblock` (a `<pre>` with a filename / language bar).
- **Article furniture:** `.article-title`, `.lede` for the header block.

See `preview/Type-Prose.html`, `preview/Prose-Headings.html`, `preview/Reading-Measure.html`, `preview/Prose-Elements.html`, `preview/Prose-Blocks.html`.
