...
const SURFACE_ORDER = (function () {
  ...
  (window.REG.GROUPS || []).forEach((g) => {
    if (g.subs) g.subs.forEach((sub) => (sub.items || []).forEach((id) => ids.push(id)));
    else (g.items || []).forEach((id) => ids.push(id));
  });
...
function App() {
  ...
  const go = (id) => {
    ...
    if (!L) return;
    if (L.kind === "screen" || L.kind === "panel") nav(L.route);
    else if (L.kind === "comp") { nav(L.route); const s = window.REG.SURF[id]; toast(`${id} · ${s.name} — trên màn ${L.where}`, "info"); }
    else if (L.kind === "modal") navOpen(L.route, L.type, L.ctx ? L.ctx() : {});
    else if (L.kind === "preview") navPreview(L.route, L.party);
  };
  ...
  aUseEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === "ArrowRight") { e.preventDefault(); stepSurface(1); }
      else if (e.key === "ArrowLeft") { e.preventDefault(); stepSurface(-1); }
      else if (e.key === "h" || e.key === "H") { e.preventDefault(); setCleanNavHidden((v) => !v); }
    };
