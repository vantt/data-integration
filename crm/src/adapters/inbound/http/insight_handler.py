...
def wire_insight_router(party_repo: PartyRepository, cache_repo: CacheRepository) -> None:
    """Bind repositories; called during app startup."""
    global _party_repo, _cache_repo
    _party_repo = party_repo
    _cache_repo = cache_repo


def _repos() -> tuple[PartyRepository, CacheRepository]:
    if _party_repo is None or _cache_repo is None:
        raise RuntimeError("InsightHandler repositories not wired")
    return _party_repo, _cache_repo
