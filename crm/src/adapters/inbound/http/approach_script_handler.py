...
def wire_approach_script_router(
    party_repo: PartyRepository,
    approach_repo: ApproachScriptRepository,
) -> None:
    """Bind repositories; called during app startup."""
    global _party_repo, _approach_repo
    _party_repo = party_repo
    _approach_repo = approach_repo


def _repos() -> tuple[PartyRepository, ApproachScriptRepository]:
    if _party_repo is None or _approach_repo is None:
        raise RuntimeError("ApproachScriptHandler repositories not wired")
    return _party_repo, _approach_repo
...
def get_approach_script(party_id: str):
    ...
    """
    party_repo, approach_repo = _repos()
