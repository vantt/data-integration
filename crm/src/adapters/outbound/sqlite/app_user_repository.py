...
class SQLiteAppUserRepository:
    ...
    _SELECT = (
        "SELECT user_id, staff_id, email, full_name, role, is_active, created_at, updated_at, lark_user_id "
        "FROM crm_app_user"
    )
    ...
    def create(self, user: AppUser) -> AppUser:
        ...
        sql = (
            ...
            "(user_id, staff_id, email, full_name, role, is_active, created_at, updated_at, lark_user_id) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
        with self._conn:
    ...
    def update(self, user_id: str, **kwargs: object) -> None:
        ...
        _ALLOWED = {"email", "full_name", "role", "is_active", "updated_at", "lark_user_id", "staff_id"}
