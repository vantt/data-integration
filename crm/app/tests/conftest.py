"""conftest.py — shared pytest fixtures for crm/app/tests/.

sys.path setup:
  - repo root (data-integration/) for crm.app.* namespace
  - crm/app/ for bare domain.* / application.* imports used inside app modules
...
_PYTHON_ROOT = str(pathlib.Path(__file__).parents[1]) # .../crm/app
